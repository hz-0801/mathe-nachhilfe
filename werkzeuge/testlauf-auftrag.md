Vorlage; für einen Lauf in die Wurzel als auftrag-testlauf.md kopieren, Datum eintragen.
...
`blaetter/testlauf-<datum>/` mit dem Datum des Laufs (Tag des
Starts, aus `Get-Date`, nicht aus einem Text übernommen). Gibt es
den Ordner schon (zweiter Lauf am selben Tag), wird ein „b“
angehängt. Darin:
...
3. Bauweise festlegen und in `stand.md` notieren – zuerst der
   Regelweg, der Ersatzweg nur als zweite Stufe:
   - Regelweg (erste Stufe; `claude` ist seit dem 25.09.2026 am
     Rechner angemeldet): je Eingabe ein Aufruf
     ...
     er „Not logged in“ oder bricht er ab, gilt der Regelweg als
     nicht verfügbar.
   - Ersatzweg (zweite Stufe, nur wenn der Regelweg nicht
     verfügbar ist oder die Systemanweisung nicht annimmt): je
     Eingabe ein Sub-Agent (Task) mit der Nachricht aus
     `werkzeuge/testlauf-umgebung.md` (Prompttext als erster Teil,
     Eingabe als zweiter Teil); ein Sub-Agent je Eingabe, nie zwei
     in einem.
   - In beiden Wegen laufen die Blätter **nacheinander**, nie
     gleichzeitig: die nächste Sitzung startet erst, wenn die
     vorige beendet und abgelegt ist (Teil 2). Grund: der Lauf vom
     25.09.2026 startete alle zehn parallel; die Sitzungen teilten
     sich den Ordner und legten Irrläufer in der Repo-Wurzel ab.
...
Für jede Zeile der CSV in Reihenfolge, eine nach der anderen (die
nächste Zeile erst nach Schritt 6 der vorigen):
...
2. Bericht `bericht-testlauf-<datum>.md` in der Wurzel (Datum des
   Laufs wie der Ordnername, nicht das Datum des Berichts): erste
...
Start hh:mm · Ende hh:mm · <Grund bei offen>". Kopf: Datum,
Prompt-Version, Bauweise, Modell. Wird nach jedem Schritt
geschrieben, nie nur am Ende. Uhrzeiten und Datum nur aus
`Get-Date` (in dem Augenblick abgefragt, in dem der Schritt beginnt
oder endet), nie aus einem Text, einer Sitzungsausgabe oder dem
