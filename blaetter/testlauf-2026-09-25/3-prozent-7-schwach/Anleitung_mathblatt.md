...
Soll der Schüler selbst einen Winkel antragen, nimmst du `\winkelstrahl` und zeichnest den Strahl nie selbst mit `tikz`: das Makro setzt den Scheitel an den linken Rand, beschriftet ihn und hält über dem Strahl 4 cm frei, damit der zweite Schenkel mit dem Geodreieck Platz hat. Der Aufgabentext steht davor, das Makro bildet einen eigenen Absatz.
...
`\saeulenab`, `\balkenab` und `\liniendia` ergänzen `\saeulen` um Diagramme zum Ablesen: keine Wertebeschriftung an der Säule/dem Balken/dem Punkt, dafür ein feines Hilfsgitter zwischen den bezifferten Linien (Schlüssel `yfein` bzw. `xfein`), von dem der Wert abgelesen wird. Leerer Wert (`Mi/`) lässt die Säule weg, nur Achse und Gitter bleiben (Schüler zeichnet selbst). `ymin` (Voreinstellung 0) setzt den Startwert der Achse für Aufgaben, bei denen die Achse nicht bei null beginnt; `ohnezahlen` unterdrückt zusätzlich die Achsenbezifferung ganz, für Aufgaben zur Achseneinteilung selbst. `breite`/`hoehe` skalieren Säulen- bzw. Balkenbreite in cm; bei `\saeulenab` ist `breite` nur ein Mindestmaß: Das Makro misst die Kategorienamen selbst, rückt die Säulen so weit auseinander, dass der breiteste Name mit Lücke danebenpasst, und bricht die Namen um, wenn der Satzspiegel dafür nicht reicht – du setzt `breite` also nicht mehr von Hand hoch, nur noch, wenn die Säulen aus anderen Gründen breiter sein sollen. `\liniendia` nutzt dieselben Schlüssel wie `\saeulenab` (`ymax`, `ystep`, `yfein`, `ylabel`, `breite`), zeichnet aber Punkte über den Kategorien und verbindet sie; die y-Achse beginnt hier immer bei 0. Bei `\liniendia` ist `breite` (Abstand der Punkte) genauso ein Mindestmaß wie bei `\saeulenab`: lange Kategorienamen rücken die Punkte auseinander und werden umbrochen, wenn der Satzspiegel dafür nicht reicht. Bei `\balkenab` stehen die Namen links an der Achse; dort ist `breite` (Balkenlänge bei `xmax`) ein Höchstmaß: Das Makro misst die Namen und den Achsentitel, hält links den Platz für den breitesten Namen frei und kürzt die Balken so weit, dass die Grafik im Satzspiegel bleibt. Bräuchte ein Name mehr als 45 % der Zeilenbreite, wird er umbrochen und die Zeilen rücken weiter auseinander. Lange Kategorienamen sind also bei allen dreien einfach hinzuschreiben – kein Kürzen, kein Nachstellen von `breite` oder `hoehe`, und keine Zeilenumbrüche von Hand im Namen.
