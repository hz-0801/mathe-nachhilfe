...
- **Typname mit Gegenstandsklasse** (Entscheidung 24, `iqb.md` § 6): Bei Themen mit Klassenliste beginnt der Typname mit der Klasse und Doppelpunkt („Matrizenalgebra: Alle Fixvektoren einer Matrix ermitteln"), bei allen anderen ohne Präfix; maßgeblich ist das Thema des Typs in `iqb-typen.csv`. Das Skript prüft das. Der Schnitt Thema × Klasse × Handlung (aus `format`) ist die Einheit für den Blattbau in Teil A; der Typ nach Kern § 6 bleibt Feinetikett.
- **Abgleichlauf nach jedem Stapel** (Kern § 9): Etiketten des Stapels gegen `iqb-typen.csv` vereinheitlichen; Umbenennungen und Zusammenziehungen nur über `iqb-abgleich.py` (Regeln im Skript, danach Selbstprüfung), Liste alt → neu in `iqb-pruefungen.md` § 5. Commit je Stapel nach bestandener Selbstprüfung; Nachführen wie bei abi, dazu die Eichungsquote in `iqb-pruefungen.md` § 4.
