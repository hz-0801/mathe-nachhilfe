...
| `katalog-prompt.md` | Kern der Erfassungsmethode (v0.4, Schema-Version 2): Zeilenregel, 37 Felder mit Kopfzeile, Formvokabular, Handlung je format (§ 5), Markierungen in bemerkung („Dublette von:", „Traegerbindung: Kontext"), Prüfung, Ausgabe je Heft. Prüfungsunabhängig; Etikettenänderungen nur im Abgleichlauf (§ 6, § 9). |
| `abi.md` | Profil abi (v0.9): Prüfung, Basis-URL, Heftaufbau, Kürzel und id-Muster, Sachgebiete (`leitidee`), Verweis auf das Vokabular, Besonderheiten beim Erfassen, Beispielzeilen, Offenes. **Bei Widerspruch zum Kern gilt das Profil.** |
