...
| `katalog-prompt.md` | Kern der Erfassungsmethode (v0.5, Schema-Version 2): Zeilenregel, 37 Felder mit Kopfzeile, Formvokabular, Handlung je format (§ 5), Markierungen in bemerkung („Dublette von:", Vorstufe „Poolaufgabe (nicht erfasst):" als Übergangszustand, „Abgewandelt von:", „Traegerbindung: Kontext"), Prüfung, Ausgabe je Heft. Prüfungsunabhängig; Etikettenänderungen nur im Abgleichlauf (§ 6, § 9). |
| `abi.md` | Profil abi (v0.13): Prüfung, Basis-URL, Heftaufbau, Kürzel und id-Muster, Sachgebiete (`leitidee`), Verweis auf das Vokabular, Besonderheiten beim Erfassen, Beispielzeilen, Offenes. **Bei Widerspruch zum Kern gilt das Profil.** |
...
- **Qualitätsschranke im Skript.** `SCHWELLEN` in `iqb-bau.py` (`iqb.md` § 7): Zeilen mit „?", Anteil neuer Typen, Zeilen ohne passendes Thema. Reißt eine Schranke, werden Etiketten und Rechnungen geprüft und der Lauf wiederholt; erst wenn das nicht hilft, wird der Wert im Skript geändert und in `iqb-pruefungen.md` § 5 begründet. Seit iqb-bau.py v1.4 kann `KONFIG` die Eichschwelle für einen Stapel überschreiben (`eichung_mindestens` mit `eichung_grund`). `probe: True` prüft alles und schreibt nichts; damit wird eine einzelne Aufgabe ausprobiert, ohne einen Zwischenstand zu erzeugen.
