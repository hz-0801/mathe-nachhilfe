...
Version 0.1 · 13.09.2026 · gilt mit katalog-prompt.md v0.3 und iqb.md v0.1
...
KONFIG = {
    ...
    "soll": {
        "2026MgrundlegendAAnalysis11": 5,
    },
    # Seitenzahl je Datei, wenn sie von 2 abweicht (Teil A hat zwei Seiten)
    "seiten": {},
...
QUELLEN_HEAD = ["kennung", "jahr", "niveau", "teil", "sachgebiet", "gruppe",
                "hilfsmittel", "nr", "papier", "stapel"]

PFLICHT = ("id jahr papier block aufgabe titel teilaufgabe seite punkte hilfsmittel afb_amtlich "
...
def row(kennung, teilaufgabe, **kw):
    ...
    kommen aus der Kennung (iqb-quellen.csv); in Teil B ist innen= die
    Aufgabennummer innerhalb der Datei (vorläufig, iqb.md § 9)."""
    if kennung not in QUELLE:
        sys.exit(f"{kennung}: nicht in {QUELLEN}")
    q = QUELLE[kennung]
    innen = kw.pop("innen", "")
    z = {k: "" for k in HEAD}
    z.update(id=f"{kennung}-{innen}{teilaufgabe}", jahr=q["jahr"], papier=q["papier"],
...
def pruefe_zeile(z, a):
    ...
    grenze = KONFIG.get("seiten", {}).get(kennung, 2) if z["block"] == "A" else 99
    for s in z["seite"].split("|"):
        if s.isdigit():
            a(int(s) <= grenze, f"{i}: Seite {s} größer als der Dateiumfang")
...
def main():
    ...
    if not ZEILEN:
        ...
        # Stapelvollständigkeit im Bestand: jede angefangene Datei ganz, jeder
        # angefangene Stapel ganz
        ...
        for s in sorted(stapel):
            fehlt = [k for k, q in QUELLE.items() if q["stapel"] == s and k not in kennungen]
