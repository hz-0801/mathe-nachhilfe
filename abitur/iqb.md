...
- **Qualitätsschranke im Skript, nicht im Urteil des Lehrers.** Der Lehrer liest
  ...
  · Eichung: mindestens 85 % der Zeilen treffen nach der engen Fassung den
    ...
    **Dokumentierte Unterschreitung** (Beschluss vom 26.09.2026,
    beschluss-2026-09-26.md Punkt 2; iqb-bau.py v1.10): Der Delta-Stapel
    2018-ea-B (CAS) ist mit dokumentierter Unterschreitung der Eichschwelle
    erfasst – 69 von 85 Zeilen (81 %) nach der engen Fassung mit Deutungsliste
    (f); erster vollständiger Stand 57, nach Prüfung der Abweichungen 67, (f)
    brachte zwei (iqb-pruefungen.md § 4). Die Schwelle bleibt 85 % für alle
    Stapel; sie zu senken hieße, den Maßstab dem Ergebnis anzupassen. Umsetzung:
    Die Liste EICHUNG_UNTERSCHRITTEN in iqb-bau.py nennt nur diesen Stapel, mit
    seinem gemessenen Wert (69 von 85) und dem Verweis auf den Beschluss; der
    Stapellauf macht aus der gerissenen Schwelle nur dann eine Warnung, wenn die
    Messung genau diesen Wert ergibt, sonst bleibt es ein Fehler, und die
    Selbstprüfung meldet den Stapel als Warnung mit dem Wert aus dem Bestand.
    Das ist keine Wiedereinführung der Überschreibung je Stapel aus v1.4: kein
    anderer Grenzwert, kein KONFIG-Feld, die Unterschreitung bleibt mit ihrem Wert
    sichtbar statt in einer gesenkten Schwelle zu verschwinden. Ein weiterer
    Eintrag setzt einen eigenen Beschluss voraus.
